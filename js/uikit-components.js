{
    const {String: t, Boolean: e, Number: i} = window
      ...
      , r = (()=>{
        const t = h.mixins[0]
          , {boxModelAdjust: n, css: r, dimensions: a, endsWith: l, height: d, isNumeric: c, isString: u, isVisible: m, offset: p, query: g, toFloat: f} = UIkit.util;
        return s({
            mixins: [t, o],
            props: {
                expand: e,
            ...
            update: {
                read({minHeight: t}) {
                    ...
                    if (this.expand)
                        e = d(window) - (a(document.documentElement).height - a(this.$el).height) - i || "";
                    else {
                        if (e = "calc(100vh",
                        this.offsetTop) {
                            const {top: t} = p(this.$el);
                            e += t > 0 && t < d(window) / 2 ? ` - ${t}px` : ""
                        }
                        !0 === this.offsetBottom ? e += ` - ${a(this.$el.nextElementSibling).height}px` : c(this.offsetBottom) ? e += ` - ${this.offsetBottom}vh` : this.offsetBottom && l(this.offsetBottom, "px") ? e += ` - ${f(this.offsetBottom)}px` : u(this.offsetBottom) && (e += ` - ${a(g(this.offsetBottom, this.$el)).height}px`),
                        e += (i ? ` - ${i}px` : "") + ")"
                    }
                    return {
                        minHeight: e,
                ...
                write({minHeight: t, prev: e}) {
                    ...
                    this.minHeight && f(r(this.$el, "minHeight")) < this.minHeight && r(this.$el, "minHeight", this.minHeight)
                },
                events: ["resize"]
            }
        })
    ...
    const l = (()=>{
        ...
        return {
            connected() {
                const e = this.$el
                  , o = this.buttonDown = t("<button class=\"input-number-down input-number-btn btn\"><span class='unicon-subtract'></span></button>")
                  , h = this.buttonUp = t("<button class=\"input-number-up input-number-btn btn\"><span class='unicon-add'></span></button>");
                i(e, o),
                s(e, h);
                const r = (t,e)=>{
                    if ("+" === e && t.stepUp)
                    ...
                    const i = +(t.step || "1") * ("-" === e ? -1 : 1)
                      , s = +(t.value || "0") + i;
                    "-" === e && t.min && s < +t.min || "+" === e && t.max && s > +t.max || (t.value = s)
                }
                ;
                n(o, "click", (t=>{
                    t.preventDefault(),
    ...
    n("countdown", (()=>{
        ...
        return {
            ...
            computed: {
                ...
                minutes: ({clsWrapper: t},e)=>s(t.replace("%unit%", "minutes"), e),
                seconds: ({clsWrapper: t},e)=>s(t.replace("%unit%", "seconds"), e),
                unitValueElement: ({clsWrapper: t},e)=>s(t.replace("%unit%", "unit-value"), e),
            ...
            disconnected() {
                this.stop(),
                this.oneUnit ? (o(this.unitValueElement),
                o(this.labelElement)) : this.units.forEach((t=>this[t] && o(this[t]))),
            ...
            update: {
                write() {
                    ...
                    if (t.total <= 0 && (this.stop(),
                    t.days = t.hours = t.minutes = t.seconds = 0,
                    this.showEnd.forEach((t=>t.classList.remove("uk-hidden"))),
                    this.hideEnd.forEach((t=>t.classList.add("uk-hidden")))),
                    this.oneUnit) {
                        const e = this.units.find(((e,i,s)=>Math.floor(t[e]) > 0 || i === s.length - 1));
                        return this.labelElement.textContent = 1 === Math.floor(t[e]) ? a[e] : e,
            ...
            methods: {
                setUnit(e, i, s) {
                    ...
                    n.length !== i.children.length && h(i, n.map((()=>"<span></span>")).join("")),
                    n.forEach(((t,e)=>i.children[e].textContent = t)))
                },
                start() {
                    this.stop(),
                    this.date && this.units.length && (this.showEnd.forEach((t=>t.classList.add("uk-hidden"))),
                    this.$update(),
    ...
    const c = UIkit.component("drop").options
      , u = (()=>{
        const [s,n,o] = c.mixins
          , {addClass: h, append: r, apply: a, css: l, hasClass: d, includes: u, isTouch: m, MouseTracker: p, offset: g, on: f, once: $, parent: w, pointerCancel: b, pointerDown: k, pointerEnter: v, pointerLeave: y, pointerUp: E, query: U, removeClass: x, toggleClass: T, within: D} = UIkit.util;
        let I;
        return {
            mixins: [s, n, o],
            args: "pos",
            ...
            computed: {
                boundary: ({boundary: t},e)=>!0 === t ? window : U(t, e),
                clsDrop({clsDrop: t}) {
            ...
            connected() {
                ...
                this.toggle && !this.target && (this.target = this.$create("toggle", U(this.toggle, this.$el), {
                    target: this.$el,
                    mode: this.mode
                }))
            },
            ...
            events: [{
                ...
                name: `${v} focusin`,
                filter() {
                    return u(this.mode, "hover")
                },
                handler(t) {
                    m(t) || this.clearTimers()
                }
            }, {
                name: `${y} focusout`,
                filter() {
                ...
                handler() {
                    I = this,
                    this.tracker.init(),
                    $(this.$el, "hide", f(document, k, (({target: t})=>!D(t, this.$el) && $(document, `${E} ${b} scroll`, (({defaultPrevented: e, type: i, target: s})=>{
                        e || i !== E || t !== s || this.target && D(t, this.target) || this.hide(!1)
                    }
                    ), !0))), {
                        self: !0
                    }),
                    $(this.$el, "hide", f(document, "keydown", (t=>{
                ...
                name: "hide",
                handler({target: t}) {
                    this.$el === t ? (I = this.isActive() ? null : I,
                    this.tracker.cancel()) : I = null === I && D(t, this.$el) && this.isToggled() ? this : I
                }
            }],
            update: {
                write() {
            ...
            methods: {
                show(t=this.target, e=!0) {
                    if (this.isToggled() && t && this.target && t !== this.target && this.hide(!1),
                    this.target = t,
                    this.clearTimers(),
                    !this.isActive()) {
                        if (I) {
                        ...
                        this.container && w(this.$el) !== this.container && r(this.container, this.$el),
                        h(this.$el, "uk-open"),
                        this.position(),
                        x(this.$el, "uk-open"),
                ...
                hide(t=!0) {
                    ...
                    this.isDelaying = function(t) {
                        const e = [];
                        return a(t, (t=>"static" !== l(t, "position") && e.push(t))),
                        e
                ...
                position() {
                    x(this.$el, `${this.clsDrop}-stack`),
                    T(this.$el, `${this.clsDrop}-boundary`, this.boundaryAlign);
                    const t = g(this.boundary)
                      , e = this.boundaryAlign ? t : g(this.target);
                    if ("justify" === this.align) {
                        const t = "y" === this.getAxis() ? "width" : "height";
                        l(this.$el, t, e[t])
                    } else
                        this.boundary && this.$el.offsetWidth > Math.max(t.right - e.left, e.right - t.left) && h(this.$el, `${this.clsDrop}-stack`);
            ...
            update: {
                read() {
                    ...
                    if (i) {
                        const {x: e, width: s} = t.getBoundingClientRect()
                          , n = [e, s].join(",");
                        n === this.lastSeenElPos && (i = !1),
                        this.lastSeenElPos = n
                    }
                    return {
                        needUpdate: i
                    }
                ...
                write({needUpdate: t}) {
                    ...
                    this.ulList.forEach((t=>{
                        ...
                        t.getBoundingClientRect().right > e && n(t, "reverse-submenu-open")
                    }
                    ))
                },
                events: ["resize"]
